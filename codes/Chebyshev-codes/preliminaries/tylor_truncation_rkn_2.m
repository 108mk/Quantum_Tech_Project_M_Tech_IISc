...
%% Discretize Burger's equation BOUNDARY CONDITION
...
for N = [Ks(K_max)]
    ...
    plot(xs,reshape(ys_c_N(N, nt*1/100, 1:nx),nx,1),'-*','DisplayName',sprintf('Carleman solution at $t=500$, $N=%d$',N));
end
...
for N = [Ks(K_max)]
    ...
    plot(xs,reshape(ys_c_N(N, nt*4/10, 1:nx),nx,1),'-*','DisplayName',sprintf('Carleman solution at $t=2000$, $N=%d$',N));
end
...
for N = [Ks(K_max)]
    ...
    plot(xs,reshape(ys_c_N(N, nt*8/10, 1:nx),nx,1),'-*','DisplayName',sprintf('Carleman solution at $t=3000$, $N=%d$',N));
end
