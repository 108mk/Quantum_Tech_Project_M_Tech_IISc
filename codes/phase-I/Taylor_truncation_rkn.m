...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
nx = 16; % Spatial discretization for Euler's method for ODE
...
K_max = 2; %taylor truncation order (reduncdance)
Ks = 1:K_max; % taylor truncation levels (redundance)
...
for N = 1:K_max
    ...
    for k = 1:nt
        eps_c_d_N(N,k) = norm(reshape(dus_c_d_N(N,k,:),nx,1), 2); %% NORM %%%%%%%%%%%%%%%%
        eps_rel_c_d_N(N,k) = norm(reshape(dus_rel_c_d_N(N,k,:),nx,1), 2);

        eps_c_pde_N(N,k) = norm(reshape(dus_c_pde_N(N,k,:),nx,1));
        eps_rel_c_pde_N(N,k) = norm(reshape(dus_rel_c_pde_N(N,k,:),nx,1), 2);

        eps_d_pde(k) = norm(reshape(dus_d_pde(k,:),nx,1));
        eps_rel_d_pde(k) = norm(reshape(dus_rel_d_pde(k,:),nx,1), 2);

        eps_d_e(k) = norm(reshape(dus_d_e(k,:),nx,1));
        eps_rel_d_e(k) = norm(reshape(dus_rel_d_e(k,:),nx,1), 2);
    end
end
...
for N = [Ks(K_max)]
    ...
    plot(xs,reshape(ys_c_N(N, 500, 1:nx),nx,1),'-*','DisplayName',sprintf('Carleman solution at $t=500$, $N=%d$',N));
end
...
for N = [Ks(K_max)]
    ...
    plot(xs,reshape(ys_c_N(N, 2000, 1:nx),nx,1),'-*','DisplayName',sprintf('Carleman solution at $t=2000$, $N=%d$',N));
end
...
for N = [Ks(K_max)]
    ...
    plot(xs,reshape(ys_c_N(N, 4000, 1:nx),nx,1),'-*','DisplayName',sprintf('Carleman solution at $t=3000$, $N=%d$',N));
end
...
ax = subplot(2,2,4);
semilogy(Ks, (1/(max(eps_c_d_N,[],2)) ),'-o','DisplayName',sprintf('Time-maximum error'));
