...
nt = 4000; % Temporal discretization for Euler's method for ODE
...
beta = -0.4; % Linear damping coefficient
f = 1; % Number of oscillations of the sinusoidal initial condition inside the domain  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  edited to 2.

T = 3; % Simulation time

flag_source = 1; %source term indicator
F0_fun = @(t,x) U0*exp(-(x-L0/4).^2/(2*(L0/32)^2)).*cos(2*pi*t)*(flag_source-1); % Source function.
...
nu = 0.05; %U0*L0/Re0; % Viscosity/Diffusion cefficient
...
F1(1:nx+1:end) = -(2)*nu/dx^2;    %%%%%%%%%%%%%%%%%%%% edited
...
burger_pde = @(x,t,u,dudx) deal(1, nu*dudx, -beta*u + F0_fun(t,x));    %%%%%%%%-----edited------main
burger_ic = u0;
