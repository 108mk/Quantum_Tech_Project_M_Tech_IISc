...
Ns = 1:N_max; % Carleman levels

ode_deg = 2; % Degree of the Carleman ODE, should not be changed

K_max = 3; % Taylor truncation
Ks = 1:K_max; % Truncation levels
...
for N = Ks
    A_N = A; % max truncated carleman matrix
    
    %%% preparing TAYLOR series terms
    WK = zeros(dNs(N_max), dNs(N_max)); % TYLOR product matrix initialize
    A_N_dt = dt*A_N; %time multiplied matrix
...
us_c_N = ys_c_N(:,:,1:nx); % extracting solution from carleman emdedding
...
for N = 1:K_max
    ...
    for k = 1:nt
        eps_c_d_N(N,k) = norm(reshape(dus_c_d_N(N,k,:),nx,1), pp); %% NORM %%%%%%%%%%%%%%%%
        eps_rel_c_d_N(N,k) = norm(reshape(dus_rel_c_d_N(N,k,:),nx,1), pp);

        eps_c_pde_N(N,k) = norm(reshape(dus_c_pde_N(N,k,:),nx,1));
        eps_rel_c_pde_N(N,k) = norm(reshape(dus_rel_c_pde_N(N,k,:),nx,1), pp);

        eps_d_pde(k) = norm(reshape(dus_d_pde(k,:),nx,1));
        eps_rel_d_pde(k) = norm(reshape(dus_rel_d_pde(k,:),nx,1), pp);

        eps_d_e(k) = norm(reshape(dus_d_e(k,:),nx,1));
        eps_rel_d_e(k) = norm(reshape(dus_rel_d_e(k,:),nx,1), pp);
    end
end
...
for N = [Ks(K_max)]
    ...
    plot(xs,reshape(ys_c_N(N, 50, 1:nx),nx,1),'-*','DisplayName',sprintf('Carleman solution at $t_n=50$, $N=%d$',N_max));
end
%%%%%%%%%
%%%%%%%%%%
...
for N = [Ks(K_max)]
    ...
    plot(xs,reshape(ys_c_N(N, 500, 1:nx),nx,1),'-*','DisplayName',sprintf('Carleman solution at $t_n=500$, $N=%d$',N_max));
end
...
for N = [Ks(K_max)]
    ...
    plot(xs,reshape(ys_c_N(N, nt*8/10, 1:nx),nx,1),'-*','DisplayName',sprintf('Carleman solution at $t_n=3000$, $N=%d$',N_max));
end
...
for N = Ks
    ax = subplot(2,2,3);
    ax.ColorOrderIndex = N;
    semilogy(ts,eps_c_d_N(N,:),'DisplayName',sprintf('Tylor truncation, $K=%d$',N));
    hold on;
end
...
sgtitle(sprintf('Reaction-Diffusion-solution with $$n_x=%d$, $n_t=%d$, $\\mathrm{R}=%.2f$',nx,nt, R), 'interpreter','latex', 'fontsize', fontsize+2);
%savefig(sprintf('rde_re0_%.2f_N_%d_nx_%d_nt_%d_rev2.fig',Re0,N_max,nx,nt));

temp = ys_c_N(K_max, 1:nt, 1:nx);
temp1 = squeeze(temp);
figure(2)
mesh(temp1);
...
ylabel('$\textbf{t}$', 'interpreter','latex');
zlabel('$u(x,t)$', 'interpreter','latex');

% Plot time-maximum absolute l_2 error between Carleman and pdepe
%figure(3)
%semilogy(Ks, (1./(max(eps_c_d_N,[],2)) )) %%%%####&&&&@@@@@@@@\
%xlabel('$K$', 'interpreter','latex')
%ylabel('$\frac{1}{\epsilon}$', 'interpreter','latex');


% % % % % temp = ys_c_N(1, 1:nt, 1:nx);
% % % % % temp1 = squeeze(temp);
% % % % % figure(3)
% % % % % mesh(temp1);
% % % % % 
